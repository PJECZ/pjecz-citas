...
function conectarse(){
    ...
    // Configuración db.ini
    $db_ini = parse_ini_file('db.ini', true)
    if ($_SESSION['entorno']=="Local") {
        $servidor = $db_ini["Local"]["servidor"];
        $usuario = $db_ini["Local"]["usuario"];
        $passwd = $db_ini["Local"]["passwd"];
        $database = $db_ini["Local"]["database"];
    } elseif ($_SESSION['entorno']=="Internet") {
        $servidor = $db_ini["Internet"]["servidor"];
        $usuario = $db_ini["Internet"]["usuario"];
        $passwd = $db_ini["Internet"]["passwd"];
        $database = $db_ini["Internet"]["database"];
    } else {
        die("No está configurado el entorno en db.ini")
    }
